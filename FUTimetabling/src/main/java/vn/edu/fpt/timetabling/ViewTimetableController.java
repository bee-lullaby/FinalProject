...
public class ViewTimetableController {
	...
	public String viewTimetableStaffInit(Model model, HttpSession httpSession) {
		...
		return "redirect:/staff/viewTimetable?semesterId=" + semesterId + "&classSemesterId=0";
	}

	@RequestMapping(value = "/staff/viewTimetable", method = RequestMethod.GET, params = { "semesterId" })
	public String viewTimetableStaff(@RequestParam int semesterId,
			@RequestParam(value = "studentId", required = false) Integer studentId,
			@RequestParam(value = "teacherSemesterId", required = false) Integer teacherSemesterId,
			@RequestParam(value = "classSemesterId", required = false) Integer classSemesterId, Model model,
			HttpSession httpSession) {
		if (studentId == null && teacherSemesterId == null && classSemesterId == null) {
			...
			if (studentId != 0) {
				...
				Semester s = semesterService.getSemesterById(semesterId, false, false, false, false);
				...
				model.addAttribute("listTimetables", timetableService.listTimetableByStudent(semesterId, studentId));
			} else {

				List<Student> list = studentService.listStudents();
				if (list != null && !list.isEmpty() && list.get(0) != null) {
					return "redirect:/staff/viewTimetable?semesterId=" + semesterId + "&studentId="
			...
			if (teacherSemesterId != 0) {
				...
				model.addAttribute("listTimetables", timetableService.listTimetableByTeacher(teacherSemesterId));
			} else {

				List<TeacherSemester> list = teacherSemesterService.listTeacherSemestersForView(semesterId);
				if (list != null && !list.isEmpty() && list.get(0) != null) {
					return "redirect:/staff/viewTimetable?semesterId=" + semesterId + "&teacherSemesterId="
			...
			if (classSemesterId != 0) {
				...
				model.addAttribute("listTimetables", timetableService.listTimetableByClass(classSemesterId));
			} else {

				List<ClassSemester> list = classSemesterService.listClassSemesterForView(semesterId);
				if (list != null && !list.isEmpty() && list.get(0) != null) {
					return "redirect:/staff/viewTimetable?semesterId=" + semesterId + "&classSemesterId="
	...
	public String viewTimetableTeacherInit(Model model, HttpSession httpSession) {
		...
		TeacherSemester ts = teacherSemesterService.getTeacherSemesterByTeacherSemester(id, semesterId, false, false);
		if (ts != null) {
			return "redirect:/teacherPage/viewTimetable?semesterId=" + semesterId + "&teacherSemesterId="
					+ ts.getTeacherSemesterId();
		} else {
			return "redirect:/teacherPage/viewTimetable?semesterId=" + semesterId + "&teacherSemesterId=0";
		}
	}

	@RequestMapping(value = "/teacherPage/viewTimetable", method = RequestMethod.GET, params = { "semesterId" })
	public String viewTimetableTeacher(@RequestParam int semesterId,
			@RequestParam(value = "teacherSemesterId", required = false) Integer teacherSemesterId, Model model,
			HttpSession httpSession) {
	...
	public String viewTimetableStudentInit(Model model, HttpSession httpSession) {
		...
		if (s != null) {
			return "redirect:/studentPage/viewTimetable?semesterId=" + semesterId + "&studentId=" + s.getStudentId();
		} else {
			return "redirect:/studentPage/viewTimetable?semesterId=" + semesterId + "&studentId=0";
		}
	}

	@RequestMapping(value = "/studentPage/viewTimetable", method = RequestMethod.GET, params = { "semesterId" })
	public String viewTimetableStudent(@RequestParam int semesterId,
			@RequestParam(value = "studentId", required = false) Integer studentId, Model model,
			HttpSession httpSession) {
		if (studentId == null) {
