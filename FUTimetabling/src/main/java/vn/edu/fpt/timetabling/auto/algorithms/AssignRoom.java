...
//		search_OLS();
	...
	/**Build CommonTeacherSameDayMatrix. 
	 * This function must be run after assigning teacher and AssignTeacher.findScoreElement(SingleSolution[]) 
	 * */
...
//						if (tc1 == tc2) {
	...
	/**Check if two classcourses are in the same day. 
	 * This function must be run after finding elements (AssignTeacher.findScoreElement(SingleSolution[])).*/
...
//				fsumSameNBTeacher[i] = new FuncPlus(fsumSameNBTeacher[i], x[j][i]);
			}
		...
		for (int i = 0; i < DataCenter.NBCLUSTER; i++) {
			objfunc = new FuncPlus(objfunc, fsumSameNBTeacher[i]);
		}
...
//			System.out.println("room = "+room.code+", capacity = "+room.capacity+", available-slot = "+room.availableSlot);
			System.out.println("room = "+room.code+", capacity = "+room.supplySlotCapacity+", available capacity = "+DA.availableCapacity[r]);
		}
