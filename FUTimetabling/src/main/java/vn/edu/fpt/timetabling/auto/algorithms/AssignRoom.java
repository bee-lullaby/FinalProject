...
public class AssignRoom {
	...
	public void assignRoomUsingFor(String fn_beingUsedTimeTable) {
		...
		for (int type = 0; type < DataCenter.NB_COURSETYPE; type++) {
			...
			DA.loadData_CourseRoom(
					DataCenter.DATAFOLDER + "/" + DataCenter.FILENAME_PREFIX_ROOMTYPE + (type + 1) + ".txt", type);
	...
	public void init_roomSupply() {
		...
		for (int r = 0; r < DA.nbRoom; r++) {
			...
			System.out.println("room = " + room.code + ", capacity = " + room.capacity + ", available capacity = "
					+ DA.availableCapacity[r]);
		}
