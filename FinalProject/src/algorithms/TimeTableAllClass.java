...
public class TimeTableAllClass {
	...
	public void stateModelAllClass(String fn){
		...
		//---------------------------------------------------------------------
		//constraints:
		//1. nhung mon thuoc cung 1 lop se ko conflict voi nhau: cái này đã được đảm bảo ở phase 1: OK
		
		//2. lớp A(lớp chủ) có một số môn ghép với lớp B(lớp khách) thì các môn của lớp B phải
...
//		System.out.println("Class "+DA.classes[26].code+", violation = "+calViolationOfAClass(26));
	}
...
//			bw.write(DA.nbClassCourse*NBFRAGMENTPERCOURSE);
	...
	/**Load fragment courses from file. 
	 * Build map<ClassCourse,x_day/x_slot>, map<ClassCourse,ArrayList<x_day/x_slot>>.
	 * Initialise x_day and x_slot.
	 * */
	public void loadData_FragmentANDInitializeVariables(String fn){
		int nbFC = DA.nbClassCourse*NB_FRAGMENT_PERCOURSE; //neu fc per course ko bang nhau thi problem
		/*ls = new LocalSearchManager();		
		x_day = new VarIntLS[nbFC];
		x_slot = new VarIntLS[nbFC];*/
...
//					System.out.println("fcIdx = "+fcIdx+", ccID = "+ccID);
			...
			/*for (int i = 0; i < nbFC; i++) {
				VarIntLS day = x_day[i];
				VarIntLS slot = x_slot[i];
				
				ClassCourse cc1 = mXDay2ClassCourse.get(day);
				ClassCourse cc2 = mXSlot2ClassCourse.get(slot);
				if (cc1 != cc2) {
					System.out.println("Problem in loadData_FragmentANDInitializeVariables. Not match.");
					System.exit(1);
				}
				System.out.println(i+" "+cc1.ID);
...
//				int sIdx = mXSlot2Index.get(xslot);
	...
	/** KHOI TAO:
	 * load data,
	 * make all solutions for a class,
	 * create a timetable for all class,
	 * backup current timetable,
	 * calculate supplied teacher
	 */
...
//		DA.loadData_Course_Class(sourceFn);		
		...
		for (int cl = 0; cl < DA.nbClass; cl++) {
			...
			timeTable_WareHouse[cl].D.loadData_SM(DataCenter.DATAFOLDER+"/"+DataCenter.FILENAME_PREFIX_DATAONECLASS+cl+ ".txt");
		...
		for (int cl = 0; cl < DA.nbClass; cl++) {
			...
			tt1c.findSolution(DataCenter.FILENAME_PREFIX_DATAONECLASS + cl + ".html", TimeTableAllClass.LIMIT_NBSOLUTION_PERCLASS);
//			tt1c.printHTML_all("class_" + cl + ".html",limitNbSolutionPerClass);
		}
...
//		this.loadData_supply_Teacher("teacher_supply.txt");
	...
	public int[] calTeacherDemand(SingleSolution[] pickedSolutions) {
		/* CALCULATE DEMAND OF ALL COURSES
		 * input:	timetable of all class
		 * 			supply teacher of each course
		 * output: 	demand of each course
		 */
	...
	public SingleSolution changeSession(SingleSolution singleSol){
		/* CHANGE SESSION OF A SOLUTION
		 * input: a single solution
		 * ouput: a single solution with different session
		 */
		...
		for (int d = 0; d < DA.nbDays_20; d++) {
			if (done) {
				//phai chay qua tat ca cac ngay moi chac chan duoc session bang bao nhieu
...
//					System.out.println("day = "+d+" sl = "+sl);
	...
	public int[] calculateSupplyTeacher(){
		...
		/*Random r = new Random();
		for (int c = 0; c < DA.nbCourse; c++) {
//			int nbTCperCourse = r.nextInt((2-1)+1)+1;
			int nbTCperCourse = 1;
			supply[c] = nbTCperCourse;
			System.out.println("c = "+c+", supply = "+nbTCperCourse);
		}	*/	
	...
	public int marginOfAClass(int[] demand, int[] supply, int classIdx){
		...
		entities.ClassFU cls = DA.classes[classIdx];
	...
	/** PICK MOST VIOLATED CLASS:
	 * input: 	supply teacher of each class, 			
	 * 			demand_OfACourse; 
	 * output: 	index of picked class
	 * */
...
//		supplyTeacher = calculateSupplyTeacher();
		...
		for (int cl = 0; cl < DA.nbClass; cl++) {
			ArrayList<Course> L = new ArrayList<Course>();
			entities.ClassFU cls = DA.classes[cl];
			L = DA.mClass2_CourseList.get(cls);
			for (Course course : L) {
				int cIdx = DA.mCourse2Index.get(course);
			...
			System.out.println("cl = "+cl+", margin = "+marginDemand_OfAClass[cl]+", nbViolated = "+nbOfViolatedCourse_OfAClass[cl]);
		}
...
//			int oldPosition = backUpTimeTable[pickedClass];
			int currentDelta = getAssignDelta(pickedClass,  timeTable_WareHouse[pickedClass].sol[backUpTimeTable[pickedClass]]);
			do { //find best solution for picked class
				if (next != backUpTimeTable[pickedClass]) { //ko duyet solution da co
...
//					if(newdelta < currentDelta){						
			...
			//else: unban all
			if (positionOfBest == backUpTimeTable[pickedClass]) { //better solution not found.
...
//				System.out.println("Better violation = "+getAssignDelta(pickedClass, timeTable_WareHouse[pickedClass].sol[positionOfBest]));
		...
		for (int c = 0; c < DA.nbCourse; c++) {
			System.out.println("ID = "+DA.courses[c].ID+", code = "+DA.courses[c].code+", demand = "+demand_OfACourse[c]
					+", supply = "+supplyTeacher4Optimization[c]+", margin = "+ (demand_OfACourse[c]-supplyTeacher4Optimization[c]));
		}
				...
				do {
					if (next != backUpTimeTable[pickedClass]) { //ko duyet solution da co
						...
						if (newViolation <= base_violation) {
							...
							//tinh lai room demand, neu vuot qua limit thi ko lay this solution
...
//			next = 0;			
			...
			do {				
				...
				next = 0;
				do {
//					System.out.println("class = "+pickedClass+", position = "+next);
					if (next != backUpTimeTable[pickedClass]) { //ko duyet solution da co
						//lay solution tai next
//						int nextsol = next%timeTable_AllClass[pickedClass].sol.length;
						beingUsedTimeTable[pickedClass] = timeTable_WareHouse[pickedClass].sol[next];
						...
						//tinh current score
//						current_score = 2*(newTotalDemand-bestTotalDemand) + 1*(currentRoomDemand-bestRoomDemand);
						
						//so sanh: neu new total demand < min total demand AND room demand < roomLimit
							//thi doi min. luu lai position best.
//						boolean condition1 = newTotalDemand < bestTotalDemand && currentRoomDemand < bestRoomDemand+diff;
//						boolean condition2 = newTotalDemand == bestTotalDemand && currentRoomDemand < bestRoomDemand;
						if (newTotalDemand < bestTotalDemand && currentRoomDemand < bestRoomDemand+diff) {
		...
		try {
			...
			for (int i = 0; i < 2 * DA.nbSlotsPerHalfDay; i++) {
				...
				for (int j = 0; j < DA.nbDays_20; j++) {
					...
					if (sol.T[i][j] >= 0) {
						str = "M" + sol.T[i][j];
						ClassCourse cc = DA.mID2ClassCourse_all
								.get(sol.T[i][j]);
						Course c = DA.mClassCourse2Course.get(cc);
						int idx = c.ID;
						int stt = cc.stt;
	...
	public int[][] buildConflictMatrix(SingleSolution[] sol){
		...
		for (int c1 = 0; c1 < DA.nbClassCourse-1; c1++) {		
			for (int c2 = c1+1; c2 < DA.nbClassCourse; c2++) {
				...
				ClassFU cls2 = DA.mClassCourse2Class.get(cc2);
				if (conflict[c1][c2]==1) {
//					System.out.println("conflict: <"+c1+"-"+cc1.code+"-"+cls1.code+","+c2+"-"+cc2.code+"-"+cls2.code+">");
					System.out.println("conflict: <"+c1+"-"+c2+","+cc1.code+"-"+cc2.code+","+cls1.code+"-"+cls2.code+">");
				}
			}
		}*/
...
//		TA.init("data_all_sm_merged.txt", "solutionWarehouses_improved.dat", TimeTableAllClass.NB_OF_SOLUTION_PERCLASS_LIMIT);
	}
