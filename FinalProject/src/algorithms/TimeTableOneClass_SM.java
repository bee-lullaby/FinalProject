...
//				System.out.println("c = "+D.courses[c].ID+", stt = "+D.statusOfCourse[c]);
				...
				ArrayList<FCourse> L = D.fCoursesOfCourse.get(course);
				ArrayList<FCourse> L_20 = D.fCoursesOfCourse_20.get(course);
//				System.out.println("size L = "+L.size()+", size L20 = "+ L_20.size());				
...
//				System.out.println("idx = "+fcIdx+", day = "+day+", slot = "+slot+", session = "+session);
				...
				if (day%2 == 0) {
					for (int i = 0; i < L_20.size(); i++) {
						...
						%x_day_full[fc_20].setValuePropagate(daylist_2bl_even[i]);
						%x_slot_full[fc_20].setValuePropagate(x_slot[fc].getValue());*/
...
//		D.nbDays = 4;
		...
		for (int i = 0; i < D.nbCourses; i++) {
			ArrayList<FCourse> fCourses = D.fCoursesOfCourse.get(D.classCourses[i]);
...
//		Boolean ok = solver.solveAll();
	...
	public void printHTML_all(String fn, int limit) {
		PrintWriter out = null;
...
//					out.println("");
					...
					for (int i = 0; i < 2 * D.nbSlotsPerHalfDay; i++) {
						...
						for (int j = 0; j < D.nbDays; j++) {
							...
							out.println("</td>");
//							out.print("<td height = 20 width = 20 bgcolor='green'>" + str + "</td>\n");
						}
						out.println("</tr>");
					}
					out.println("</table>");
...
//				for (int i_sl = 0; i_sl < D.nbSlotOfHalfDay * 2; i_sl++) { //for each slot sl
				for (int i_sl = 0; i_sl < D.nbSlotsPerHalfDay*2; i_sl++) { //for each slot sl
					out.println("<tr>");
...
//			out.close();
	...
	public void findSolution(String fn, int limit){
...
//								sol[n].T[i_slot][i_day] = c.ID;
					...
					for (int i = 0; i < 2 * D.nbSlotsPerHalfDay; i++) {
						...
						for (int j = 0; j < D.nbDays_20; j++) {
							...
							if(sol[n].T[i][j] >= 0 ){
								str = "M"+sol[n].T[i][j];
								ClassCourse cc = D.mID2Course.get(sol[n].T[i][j]);
								int idx = D.mCourse2Index.get(cc);
								int stt = D.statusOfCourse[idx];
								if (stt == 3) {
									out.println("bgcolor='yellow'> ");									
								} else {
...
//		out.close();
	...
	public static void main(String[] args) {
		...
		TimeTableOneClass_SM TT = new TimeTableOneClass_SM();
		TT.D = new DataOneClass();
		TT.D.loadData_SM("class0.txt");
		/*TT.D.cls = new entities.Class();
		TT.D.cls.ID = 1;
		Course[] c = TT.D.courses;
		for (Course course : c) {
